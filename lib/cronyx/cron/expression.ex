defmodule Cronyx.Cron.Expression do
  @moduledoc false
  ...
  defp parse_field(field, range) when range == 0..6 do
    if field == "6-7" do
      MapSet.new([6, 0])
    else
      parse_field_for_range(field, range)
    end
  end

  defp parse_field(field, range), do: parse_field_for_range(field, range)

  defp parse_field_for_range(field, range) do
  ...
  defp parse_part(part, range) do
    if String.contains?(part, ",") do
      Enum.flat_map(String.split(part, ","), &parse_part(&1, range))
    else
      do_parse_part(part, range)
    end
  end

  defp do_parse_part(part, range) do
    cond do
      part == "*" -> range
